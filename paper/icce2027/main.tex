...
implementation observations worth documenting in English: (i)~the
Forney deinterleaver must be aligned to the TS sync byte
\emph{before} processing, not after, and (ii)~the YTN DMB
multiplex embeds the AVC \texttt{DecoderConfigurationRecord}
inside the BIFS / scene-description stream rather than the
object-descriptor stream used in the reference MPEG-4 Systems
examples.  Neither is a standards deviation, and both were
internally known to ETRI engineers at the time of T-DMB
deployment~\cite{lee2004m4smux}; the contribution is making them
available in open source.  Source code, the reference RF capture,
...
The standards required to build such a receiver are public
(ETSI TS~102\,427/428~\cite{etsi102427,etsi102428},
ISO/IEC~14496-1, ISO/IEC 13818-1 Amendment~7), and the deployed
encapsulation path was catalogued in Korean by Lee~et~al.\ at
ETRI in 2004~\cite{lee2004m4smux}.  The contribution of this paper
is therefore an engineering rather than a discovery one: a single
black-box reconstruction of the deployed receive chain that, to
our knowledge, has not previously been released in English-language
open source.  Concretely:
...
\item A pure-Python implementation of the full T-DMB outer FEC
      ...
      on a laptop.
\item A code-form implementation of the sync-byte-aligned
      Forney deinterleaver path noted by Eo and
      Bahk~\cite{eo2024tdmb}; a clean
      0\,\%~$\to$~87.3\,\% before/after on the reference capture.
\item An empirical black-box re-derivation of the
      MPEG-4~Systems \texttt{SLConfigDescriptor} profile used by
      Korean operators (all six flag bits in a fixed pattern,
      33-bit DTS, 33-bit CTS), under which every SL packet
      header is exactly 9 bytes with byte~0 = \texttt{0xCF},
      verified against \(>\,3{,}000\) on-air PES packets.
\item Empirical observation that the YTN DMB multiplex on K8B
      embeds the AVC decoder configuration record inside the
      BIFS section stream rather than the object-descriptor
      stream prescribed by the reference MPEG-4 Systems examples
      --- a configuration choice permitted by TS~102\,428's
      stream-type allowances, not a standards deviation.
\item ``Open code + open data'' release: per-capture JSON
      metadata sidecars, a CC-BY 30~MB reference ETI file, and a
      single one-shot CLI driver, all publicly available.  Eo and
      Bahk's dmb-oss~\cite{eo2024tdmb} releases code only; their
      22-ensemble capture set is not public, so a reader outside
      Korea cannot exercise their pipeline end-to-end without
      first capturing live RF locally.  We close that gap: with
      our release a reader anywhere in the world can reproduce
      Sections~\ref{sec:rs}--\ref{sec:avcc} on a laptop, without
      RF hardware.
\end{itemize}

\emph{Context.}  Korean T-DMB is not ``DAB plus video'' but a
historically specific stack: the Eureka-147 DAB physical
layer~\cite{etsi300401} squeezed into Korea's 6\,MHz VHF TV
raster (1.728\,MHz $\times$ three sub-channels per channel),
plus an RS(204,188)+Forney outer code on the MSC, plus a
constrained MPEG-4 Systems profile carried in MPEG-2 TS per
ISO/IEC 13818-1 Amendment~7.  This layering~--- catalogued
internally at ETRI in 2004~\cite{lee2004m4smux}~--- is why an
otherwise standard DAB receiver recovers ETI frames yet still
cannot produce a video frame without the operator-specific
T-DMB stack of Sections~\ref{sec:rs}--\ref{sec:avcc} below.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/rs_phase_hist.png}
\caption{\texttt{0x47} hit counts at each of 204 candidate phases
within the post-Viterbi MSC byte stream of the K8B reference
capture.  Phase $\phi^*=160$ dominates ($25{,}006/25{,}953$
blocks, 96.3\%); all other phases are at noise level.}
\label{fig:phasehist}
\end{figure}

Figure~\ref{fig:phasehist} confirms the picture empirically for
our reference capture: one phase ($\phi^*=160$) carries 96.3\%
of the \texttt{0x47} hits while the remaining 203 candidates sit
in the noise floor.
...
\texttt{0xFA} (MPEG-4 SL-packetised, per ISO/IEC 13818-1
Amendment~7~--- the framework catalogued by Lee~et~al.~at
ETRI in 2004~\cite{lee2004m4smux}).  Each PES contains exactly
one SL packet, and each SL packet exactly one H.264 NAL unit
(stored without start-code prefix, AVCC style).

ISO/IEC 14496-1 leaves the SL header layout to a per-stream
\texttt{SLConfigDescriptor}.  In a black-box setting we cannot
read the descriptor directly, so we solve for the profile that
makes the bitstream consistent.  Combining the standard's
degrees of freedom with the FFmpeg \texttt{read\_sl\_header()}
implementation in \texttt{dmb-oss}, a 33-bit DTS + 33-bit CTS
+ six flag bits gives 72 bits = exactly 9 bytes.  We verify this
on \emph{every} one of the 3{,}135 SL packets in our reference
...
after which byte~9 is the single NAL header.  The implied
descriptor is therefore
\texttt{useAccessUnitStartFlag}=
\texttt{useAccessUnitEndFlag}=
\texttt{useTimeStampsFlag}=1
with \texttt{timeStampLength}=33; a standards-compliant operator
choice rather than a deviation.

The bare NAL stream however references PPS~0 and SPS~0, neither
of which appears in PID~\texttt{0x113}.  The reference MPEG-4
Systems examples~\cite{iso14496-1} carry the
\texttt{AVCDecoderConfigurationRecord} on the object-descriptor
(\texttt{tid}=0x04) section stream via an
\texttt{ObjectDescriptorUpdate} command (tag \texttt{0x01}); the
\texttt{dmb-oss} patch~\cite{eo2024tdmb} encodes that same
convention directly.  YTN DMB on K8B does not match it: PID
\texttt{0x111} carries only user-private commands (first body
byte \texttt{0xC0}), so a strict OD-only receiver fails with
``Unsupported OD command.''  The AVCC bytes appear instead on
PID \texttt{0x112}, where every section's first body byte is
\texttt{0x01} (\texttt{ObjectDescriptorTag}) and the descriptor
that follows is a complete \texttt{ObjectDescriptor} with the
H.264 \texttt{DecoderSpecificInfo} inside~--- a BIFS-style
\texttt{OD\_Update} placement permitted by ISO/IEC 14496-1 and
by TS~102\,428~\cite{etsi102428}.  We report this only as a
YTN-specific deployment choice; cross-checking against MBC, KBS,
or SBS captures requires data that has not been publicly
released.  Our locator scans the SD byte stream for the
...
(30~MB; $\sim$120~s; FIB CRC pass rate 75.0\%, complete ensemble
recovery; $\hat{\mathrm{SNR}}\approx 13$~dB), captured with an
Airspy~Mini and a consumer indoor flat-panel antenna with a
USB-powered amplifier from a window in Sinnonhyeon, Seoul.
Table~\ref{tab:results} summarises end-to-end yields.
...
Stage & Output \\
\midrule
ETI frames decoded                       & 5{,}016 \\
FIB CRC pass rate                        & 75.0\,\% (15{,}042 / 20{,}064) \\
Ensemble recovery                        & complete (5 services) \\
...
% ====================================================================
\section{Conclusion and Educational Use}
...
object-descriptor section stream.

Beyond the engineering documentation, we explicitly position the
artefact as a graduate-level term-project platform.  A single
eight-week project built on this codebase integrates DSP
(sampling, FFT, OFDM), RF and SDR hardware (R820T2 gain stages,
antenna design, VSWR), channel coding (CRC, Viterbi, RS,
convolutional interleaver), broadcasting (DAB frame structure,
FIC/MSC), codecs (H.264 NAL, SL packets, MPEG-2 TS) and
software-engineering practice (Python packaging, Git LFS,
licensing) into one assignment, at a per-student hardware cost
of $\approx\,\$120$.  We view our work as the SDR-based,
open-source successor to the educational T-DMB frame-analysis
simulator of Hwang and Kim~\cite{hwang2015simulator}.

All code, the reference capture, twenty decoded IDR thumbnails,
and the LaTeX source of this paper are released at
