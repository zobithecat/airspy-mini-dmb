...
TTAK.KO-07.0026~\cite{ttak0026}, ISO/IEC 14496-1~\cite{iso14496-1})
and was known internally to the engineers who designed and
deployed Korean T-DMB at ETRI and partner industry laboratories
around 2003--2007.  A reader familiar with the inside of a T-DMB
hardware decoder will find few technical surprises in the
sections that follow.

The contribution of this paper is to make that documented
behaviour available as a single, library-form, end-to-end software
artefact in a contemporary language and runtime --- and to ship it
with reproducibility scaffolding (per-capture JSON sidecars, a
public reference RF capture released under CC-BY) that to the best
of our knowledge has not been previously released in the open
...
\item An open-source Python implementation of the sync-byte-aligned
      ...
      capture.
\item A documented, code-form derivation of the 9-byte SL packet
      header layout used by Korean T-DMB video broadcasters,
      matching TTAK.KO-07.0026 and ISO/IEC 14496-1 and verified
      against $>$3000 PES packets.
\item An open-source locator and parser for the AVC
      \texttt{DecoderConfigurationRecord}, which Korean T-DMB
      multiplexers embed inside the Scene-Description
