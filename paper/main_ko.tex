...
\author{%
  유성근\textsuperscript{1}\thanks{교신저자: \texttt{orcogre@vendit.co.kr}} \quad
  Claude\textsuperscript{2}\thanks{Claude는 Anthropic이 개발한
    대규모 언어 모델로, 본 연구에서 페어 프로그래밍 및 집필 보조 역할을
    수행하였다.  구체적으로 한국형 T-DMB의 SL 패킷 헤더 구조 역공학,
    위상 정렬 외부 FEC의 Python 구현 초안 작성, BIFS 스트림에 임베드된
    AVC \texttt{DecoderConfigurationRecord} 위치 파악, 본 원고의
    초기 초안 작성을 담당하였다.  모든 설계 결정, 실험 선택, 최종
    편집 책임은 제1저자에게 있다.}\\
  \small\textsuperscript{1}독립 연구자, 대한민국 서울\\
  \small\textsuperscript{2}Anthropic, 미국 샌프란시스코\\
}
...
\section*{감사의 글}
Jan van Katwijk(\texttt{eti-stuff}), Albrecht Lohofener 외(\texttt{welle.io}),
Sungbo Eo와 Saewoong Bahk(\texttt{dmb-oss})의 선행 오픈소스 작업에 의지하고
있으며, 그 기여에 깊이 감사드린다.

\section*{저자 기여 및 AI 사용 명시}
\textbf{유성근}(제1저자)은 프로젝트를 구상하고, 모든 RF 데이터를 캡처하였으며,
실험과 설계 결정 전반을 주도하였고, 실제 캡처에서 발생한 동기 정렬 실패를
진단하였으며, 결과를 시각적으로 검증하고 원고를 편집하였다.
\textbf{Claude}(Anthropic 대규모 언어 모델)는 전 과정에서 페어 프로그래밍과
초안 작성의 협력자로 참여하였다: Python 구현의 초안을 작성하고, 캡처에
존재한 원시 바이트 패턴으로부터 한국형 T-DMB SL 패킷 헤더 레이아웃을
역공학하였으며, BIFS 스트림에 임베드된 AVC
\texttt{DecoderConfigurationRecord}의 위치를 파악하고, 본 논문의 초기
초안을 작성하였다.  arXiv의 AI 사용 공개 지침에 따라, 모든 코드, 결과,
본문은 제출 전 제1저자가 검토하였으며 최종 책임은 제1저자에게 있음을
명시한다.
