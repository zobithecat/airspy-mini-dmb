...
ISO/IEC 14496-1~\cite{iso14496-1})에 명세되어 있으며, 2003--2007년
무렵 ETRI 및 협력 산업 연구소에서 한국형 T-DMB를 설계·배치한 엔지니어들
사이에서 내부적으로 잘 알려져 있던 사실들이다.  T-DMB 하드웨어 디코더
내부 설계에 익숙한 독자라면, 이어지는 절들의 기술적 내용에서 새로운
사실을 거의 발견하지 못할 것이다.

본 논문의 기여는 그 \emph{문서화된 동작}을 동시대의 언어·런타임으로
구현한 단일 라이브러리 형태의 종단간 소프트웨어 산출물로 제공하는 데
있으며, 더불어 (저자가 아는 한) 공개된 문헌에 선례가 없는 재현성
스캐폴딩 --- 캡처별 JSON 사이드카, CC-BY로 공개되는 참조 RF 캡처 ---
을 함께 배포한다는 점에 있다.  구체적으로:
...
\item 한국형 T-DMB 비디오 방송이 사용하는 9바이트 SL 패킷 헤더 레이아웃의
       문서화·코드화: TTAK.KO-07.0026과 ISO/IEC 14496-1에 부합하며,
       3{,}000여 PES 패킷에 대해 검증.
\item AVC \texttt{DecoderConfigurationRecord}의 오픈소스 검색기·파서.
       표준 MPEG-4 Systems~\cite{iso14496-1} 예시에서는 객체기술자
       (\texttt{tid}=\texttt{0x04}) 스트림에 운반되지만, 한국형 T-DMB
       멀티플렉서는 이를 장면기술 (\texttt{tid}=\texttt{0x05})
       섹션 스트림 안에 임베드한다.
...
겨냥한 C++ T-DMB 프레임 분석 시뮬레이터를 제시한 바 있다.  본 산출물은
SDR 기반이라는 점(따라서 프레임 분석에 더해 RF, 안테나, 캡처
파이프라인까지 다룬다는 점), 완전한 오픈소스라는 점, 그리고 재생
가능한 비디오 파일까지의 종단간 처리를 다룬다는 점에서 그 선행과
구분되지만, 교육적 의도는 동일하며, 저렴한 SDR 하드웨어와 Python
과학 스택의 성숙에 힘입어 11년 뒤에 가능해진 직계 후속작으로
본 연구를 위치 짓는다.
