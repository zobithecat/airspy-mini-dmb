...
소스코드, 30\,MB 참조 캡처(CC-BY), 캡처별 메타데이터 사이드카가 모두
GitHub에 공개되어 있다 --- 의도적으로 ``코드 공개 + 데이터 공개''로
구성된 릴리스이며, 저자가 아는 한 RF 하드웨어나 한국 방문 없이도
독자가 종단간으로 재현할 수 있는 최초의 한국형 T-DMB 수신기이다.
...
소프트웨어 산출물로 제공하며, (저자가 아는 한) \emph{한국형 T-DMB
수신기를 완전히 재현 가능한 형태로 오픈소스 커뮤니티에 처음 공개}하는
사례에 해당한다.  여기서 ``완전히''라는 표현은 의도적이다: 가장 가까운
선행 연구인 Eo와 Bahk~\cite{eo2024tdmb}는 GNU Radio + FFmpeg 패치를
오픈소스로 공개하고 (본 연구도 그 FFmpeg 포크를 그대로 사용한다) 있지만,
그들이 캡처한 22 ensembles 자체는 물론 어느 단일 I/Q 혹은 ETI 파일도
공개되어 있지 않다.  따라서 한국 밖에 있는 독자는 SDR 하드웨어를 들고
한국에 와서 직접 RF를 수신해야만 그들의 파이프라인을 종단간으로 돌릴 수
있다.  반면 본 연구는 K8B의 YTN DMB 30\,MB CC-BY 참조 ETI 파일과 JSON
메타데이터 사이드카를 함께 배포한다 --- 전 세계 어디서든 저장소를 클론하고
Git~LFS에서 ETI를 받아 노트북 한 대에서 Section~\ref{sec:rs}--%
\ref{sec:avcc}의 모든 수치 결과를 RF 하드웨어나 한국 방문 없이 재현할 수
있다.  이것이 ``코드만 오픈''이 아닌 ``코드 + 데이터 모두 오픈''의
의미이다.  둘째, 본 작업을
...
\subsection{한국형 T-DMB가 왜 이렇게 생겼는가}\label{sec:why}

한국형 T-DMB는 단순히 ``DAB에 영상을 얹은 것''이라기보다,
한국의 6\,MHz VHF TV 채널 계획에 Eureka-147 DAB
물리계층~\cite{etsi300401}을 맞춰 넣고, 저전력 모바일 수신기와
방송 데이터 서비스를 위해 MPEG-4 Systems를 제한적으로 운용한
역사적 산물이다.  세 개의 층이 위로 쌓여 있다.

\begin{enumerate}[leftmargin=*,nosep]
\item \textbf{RF 래스터.}  ETSI의 1.712\,MHz Band~III 래스터가
       아니라, 한국 운용사는 6\,MHz 아날로그 TV 채널을 각각
       1.728\,MHz DMB 블록 세 개로 분할한다 (표~\ref{tab:k-block}에
       사용된 \texttt{K}$nn$\{A,B,C\} 표기).  이는 기존 한국
       TV 채널 계획을 그대로 재사용하고, 사업자별 멀티플렉스
       배분을 단순화하며, 간섭 계획을 다루기 쉽게 만들었다 ---
       대신 ETSI band-handler 표와의 호환성을 잃는다.
\item \textbf{외부 보호.}  DAB의 기본 MSC 스트림 보호는 비디오
       비트레이트에는 부족하다.  따라서 T-DMB는 MSC 서브채널
       바이트 위에 RS(204,188) 코드와 $12{\times}17$ Forney
       컨볼루션 인터리버를 추가한다 (ETSI
       TS~102\,427~\cite{etsi102427}).  본질적으로 DVB-T의 외부
       부호를 DAB 파이프에 빌려온 것이다.
\item \textbf{MPEG-2 TS 위의 제한된 MPEG-4 Systems.}  오디오·
       비디오 기본 스트림과 MPEG-4 시스템 스트림(객체기술자,
       장면기술/BIFS, Sync Layer)이 모두 ISO/IEC 14496-1의 SL로
       패킷화된 뒤, 그 SL 패킷들이 다시 \texttt{stream\_id}=
       \texttt{0xFA}로 MPEG-2 TS PES에 적재되며 (ISO/IEC 13818-1
       Amendment~7~\cite{iso13818-1-amd7}), 결과 MPEG-2 TS가
       DAB MSC 스트림 모드를 타고 흐른다.  2004년 ETRI의
       Lee~외~\cite{lee2004m4smux}는 같은 경로를 ``new DMB
       approach''로 정확히 카탈로그화하고, 같은 논문에서 채택되지
       않은 대안 멀티플렉서도 함께 제안했다.
\end{enumerate}

이 층위 구조 때문에 일반 DAB 수신기 (예: 본 연구가 1\,--\,2단계에
사용한 C++ \texttt{eti-stuff}~\cite{etistuff}) 는 ETI 프레임까지는
복원할 수 있지만, 한국형 외부 FEC, SL 패킷화, BIFS/OD 시그널링,
운용사 선택의 AVCC 배치를 처리하지 않으면 실제 비디오 프레임에는
도달하지 못한다.  이어지는 Section~\ref{sec:rs}--\ref{sec:avcc}의
모든 어려움은 위 세 층 가운데 어느 하나로 환원된다.
...
디코딩은 \texttt{dmb-oss} FFmpeg가 런타임 수정 없이 수행한다.  이 블록도는
Lee~외~\cite{lee2004m4smux}의 Figure~3(``block diagram of the precedent DMB
approach'')을 그대로 미러링한 것이며, 본 연구가 약 20년 뒤 K8B에서 실측한
MSC 스트림 모드의 ``MPEG-2 TS가 MPEG-4 SL을 캡슐화하는'' 동일한 전송 경로를
보여준다.}
\label{fig:pipeline}
\end{figure}
...
바이트 단위 컨볼루셔널 인터리빙한다.  이는 2004년 Lee~외~\cite{lee2004m4smux}가
지적한 ``동기화의 중복 \ldots\ 다중화의 중복''을 상용 프레임워크가 떠안은
대가이다: MPEG-4 SL이 MPEG-2 TS에 다시 DAB MSC에 터널링되는 구조에서는
DAB 내부 보호만으로 비디오 비트레이트를 감당할 수 없기 때문에, DVB-T의
외부 부호를 추가 계층으로 접붙인 것이다.  같은 논문의 M4SMux 제안은
이 중복 계층을 통합하려 했으나 채택되지 않았고, 상용 수신기는 결국 전체
외부 체인을 구현해야 한다.
...
\texttt{DecoderSpecificInfo} 기술자(descriptor)에 실려, 표준
\texttt{ObjectDescriptorUpdate} 명령(tag \texttt{0x01})에 포장되어
전송된다.  Eo와 Bahk의 \texttt{dmb-oss} FFmpeg 패치~\cite{eo2024tdmb}는
이 관례를 그대로 반영한다: 그들의 \texttt{m4sl\_cb}는 table-id가
\texttt{0x04}(OD 섹션)인 섹션만 처리하며, \texttt{mp4\_read\_od}는
\texttt{0x01} 이외의 명령 태그에 대해 ``\texttt{Unsupported OD command}''
오류로 중단한다.

그러나 YTN DMB K8B 캡처는 이 관례에 부합하지 않는다.  SL을 운반하는
두 PID의 모든 섹션을 검사한 결과는 다음과 같다.

\begin{itemize}[leftmargin=*,nosep]
\item \textbf{PID \texttt{0x111}} (OD, table-id~\texttt{0x04}):
       2분 캡처 동안 224개 섹션이 동일하게 반복된다.  1바이트 SL
       헤더(\texttt{0xC0}: au\_start, au\_end, 타임스탬프 없음)
       다음 바이트가 \texttt{0xC0}인데, 이는 ISO/IEC~14496-1의
       사용자 비공개 명령 영역
       (\texttt{0xC0}--\texttt{0xFE})에 속하며 표준
       \texttt{ObjectDescriptorUpdate} 태그 \texttt{0x01}이 \emph{아니다}.
       PID \texttt{0x111}의 누적 41\,kB 페이로드 어디에도 AVCC
       바이트는 나타나지 않는다.  따라서 Eo와 Bahk의
       관례~\cite{eo2024tdmb}대로 작성한 수신기는 본 캡처에서 AVCC
       추출에 실패하고 ``\texttt{Unsupported OD command: 192}''를
       출력하게 된다.
\item \textbf{PID \texttt{0x112}} (SD / BIFS, table-id~\texttt{0x05}):
       220개의 동일 섹션.  SL 헤더 바이트 직후가 \texttt{0x01} ---
       \texttt{ObjectDescriptorTag}이며, 이어지는 기술자 본문 전체가
       H.264 비디오 스트림의 \texttt{DecoderConfigDescriptor}를 포함한
       완전한 \texttt{ObjectDescriptor}이다.  그 안의
       \texttt{DecoderSpecificInfo}가 바로 우리가 찾는 AVCC 바이트이다.
\end{itemize}

다시 말해, YTN 멀티플렉스는 \texttt{ObjectDescriptor}(AVCC 내장)를
표준 OD PID의 \texttt{ObjectDescriptorUpdate}로 송출하지 않고, 장면기술
PID에 BIFS 스타일 갱신 경로로 운반한다.  두 배치 모두 MPEG-4 Systems
표준 범위 안이다 --- ISO/IEC 14496-1의 BIFS 장면기술 문법은 \texttt{OD\_Update}
명령이 BIFS 안에 포함되는 것을 명시적으로 허용하며, ETSI
TS~102\,428~\cite{etsi102428} 역시 DMB 프로파일에서 이러한 재매핑을 정당화하는
스트림 타입 제한을 허용한다.  본 연구는 다른 한국 운용사 (MBC DMB, U-KBS,
SBS u 등) 가 동일 관례를 따르는지 교차 검증할 수 없었다 --- dmb-oss 측의
캡처 셋이 공개되지 않았기 때문이다.  본 절은 이 점을 K8B 상의 YTN DMB에서
관찰된 운용사 단위 배치 선택으로만 기록한다.

실용적인 결론은 명확하다: clean-room 디코더는 OD 스트림 관례 단독에
의존할 수 없으며, BIFS 섹션 스트림으로부터도 AVCC를 복원할 수 있도록
준비되어 있어야 한다.  운용사의 IOD에 사전 접근할 수 없는 한 표준만으로는
위치를 추론할 수 없으므로, 콘텐츠를 직접 탐색해 레코드를 찾아야 한다.
